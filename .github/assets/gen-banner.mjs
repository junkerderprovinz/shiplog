/**
 * Generates the 1600x500 banners:
 *
 *   shiplog-banner.svg/.png       light, logo, name and claim  (README)
 *   shiplog-banner-dark.svg/.png  dark, logo, name and claim   (README, GitHub dark)
 *   shiplog-banner-logo.svg/.png  light, logo only             (support thread)
 *
 * Each theme embeds the logo variant that reads on its background. Text becomes
 * SVG paths, so the SVG renders the same everywhere without the fonts.
 *
...
const W = 1600, H = 500;
const LH = 400, LW = 400;
// Sizes and gaps shared by the banners of all repos.
...
// embedLogo places a logo file at (x,y,w,h) and keeps its own viewBox, since
// the masters differ in size.
...
function emit(name, svg, bg) {
  ...
  console.log(`wrote ${name}.svg and .png`);
}
...
// The support thread takes a banner without text, on the forum's light surface.
